...
const fetcha =  async (id)=>{
   ...
   const openStatus = document.querySelector('.open-status');
    ...
    galleryList.forEach(elem =>{
       
        let data = `<img loading="lazy" src="/uploads/${elem}" alt="">`
    ...
    //FOOD ITEM
    const foodPCon = document.querySelector('.food-items-div-con');
    ...
    foodList.forEach(elem =>{
       ...
       ${catData}
        ...
        <div class="food-item-details">
          <h3 class="food-item-name">${elem.cuisinename}</h3>
        
          <h3 class="about-item">Freshly made biryani from aromatic grains of rice</h3> 
          <h3 class="food-item-price">₹ ${elem.price}</h3>
        </div>
        <div class="votes-cart-con">
        <span class="food-item-rating">${elem.cuisinevotes}<i class="bi bi-chevron-double-up"></i></span>
...
const displayCart = async()=>{
    ...
    const sendData = await fetch(`/add-to-cart/${id}`, {
        method : "POST",
        headers: {'Content-Type': 'application/json'},
    ...
    const finalData = await sendData.json()

        if(finalData.foodArr.length < 1){
        ...
        console.log(finalData.foodArr);
        const cartCon = document.querySelector('.total-div');
        finalData.foodArr.forEach(elem =>{
        ...
        if(elem.category == "Veg"){
            ...
            <div class="food-item">
              ...
              <div class="right-con">
               ...
               <h4 class="food-total-price ruppee">₹ ${elem.price * elem.quantityuser}</h4>
              </div>
        ...
        grandTotal = (parseFloat(subTotal) + parseFloat(tax)).toFixed(2);
         ...
         document.querySelector('.item-count-abs').textContent =   `${finalData.foodArr.length} ${finalData.foodArr.length > 1 ? "ITEMS" : "ITEM"}`;
...
let preCookId = window.location.href.substring(window.location.href.lastIndexOf('/') + 1);

if(localStorage.getItem(preCookId)){
    displayCart()
    // addToCart(JSON.parse(localStorage.getItem(preCookId))[0])
